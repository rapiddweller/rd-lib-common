...
public abstract class NumberFormatBasedConverter<S, T> extends AbstractConverter<S, T> implements Cloneable {
  ...
  protected NumberFormatBasedConverter(Class<S> sourceType, Class<T> targetType) {
    this(sourceType, targetType, DEFAULT_DECIMAL_PATTERN);
  }

  protected NumberFormatBasedConverter(Class<S> sourceType, Class<T> targetType, String pattern) {
  ...
  protected Number parse(String input) throws ConversionException {
    ...
    try {
      ...
      throw ExceptionFactory.getInstance().conversionFailed(
          "Error parsing " + input + " as number", null);
    }
  }
